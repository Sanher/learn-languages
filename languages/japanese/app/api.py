...
def _build_meaning_match_card_for_topic(
    *,
    topic: TopicDefinition,
    level: int,
    seed: str,
    ...
    focus_items = _topic_focus_items_for_level(topic, level)
    round_data = meaning_match_service.build_round(
        focus_items=focus_items,
        seed=seed,
        item_id=activity_id,
    )
    if round_data is None:
        logger.info(
            "meaning_match_card_skipped topic=%s level=%s activity_id=%s",
            topic.topic_key,
            level,
            activity_id or "auto",
        )
    ...
    payload = {
        ...
        "prompt": round_data.prompt,
        "level": int(level),
        "payload": meaning_match_service.round_payload(round_data),
        "topic_key": topic.topic_key,
    }
    logger.info(
        "meaning_match_card_ready topic=%s level=%s activity_id=%s",
        topic.topic_key,
        level,
        round_data.item.item_id,
    )
...
def _extra_game_cards_metadata(
    *,
    ...
    if GAME_TYPE_MEANING_MATCH not in daily_game_types:
        dynamic_card = _build_meaning_match_card_for_topic(
            topic=topic,
            level=level,
            seed=f"{learner_id}:{day_iso}:{topic.topic_key}:{level}:{GAME_TYPE_MEANING_MATCH}",
        )
        if dynamic_card is not None:
            cards.append(
                {
...
def _evaluate_game_payload(
    *,
    ...
    if game_type == GAME_TYPE_MEANING_MATCH:
        return meaning_match_service.evaluate_attempt(
            MeaningMatchAttempt(
                language=language,
                item_id=payload.get("item_id", ""),
                script=payload.get("script", ""),
